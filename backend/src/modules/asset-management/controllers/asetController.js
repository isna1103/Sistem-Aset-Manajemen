const { Aset, Kategori } = require('../../shared/database');
  ...
  try {
    const data = await Aset.findOne({
      ...
      include: [
        ...
        { model: require('../../shared/database').Peminjaman, as: 'peminjaman', include: [{ model: require('../../shared/database').User, as: 'peminjam' }] },
        { model: require('../../shared/database').Maintenance, as: 'maintenance', include: [{ model: require('../../shared/database').User, as: 'teknisi' }] },
        { model: require('../../shared/database').LaporanKerusakan, as: 'laporan_kerusakan', include: [{ model: require('../../shared/database').User, as: 'pelapor' }] },
        { model: require('../../shared/database').Mutasi, as: 'mutasi', include: [{ model: require('../../shared/database').User, as: 'user' }] },
        { model: require('../../shared/database').StockOpname, as: 'stock_opname', include: [{ model: require('../../shared/database').User, as: 'pemeriksa' }] }
      ],
      order: [
        [{ model: require('../../shared/database').Peminjaman, as: 'peminjaman' }, 'created_at', 'DESC'],
        [{ model: require('../../shared/database').Maintenance, as: 'maintenance' }, 'created_at', 'DESC'],
        [{ model: require('../../shared/database').LaporanKerusakan, as: 'laporan_kerusakan' }, 'created_at', 'DESC'],
        [{ model: require('../../shared/database').Mutasi, as: 'mutasi' }, 'created_at', 'DESC'],
        [{ model: require('../../shared/database').StockOpname, as: 'stock_opname' }, 'created_at', 'DESC']
      ]
    });
    if (!data) return res.status(404).json({ message: 'Aset tidak ditemukan' });
    res.json(data);
